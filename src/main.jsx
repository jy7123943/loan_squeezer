...
const initialBuyer = {
  ...
  assets: [
    { id: 'asset-cash', name: '예금/현금', amount: 80000000 }
  ],
  netWorth: 80000000,
...
function normalizeBuyerForm(form = initialBuyer) {
  const assets = Array.isArray(form.assets) && form.assets.length > 0
    ? form.assets
    : [{ id: 'asset-cash', name: '예금/현금', amount: form.cash || initialBuyer.assets[0].amount }];
  ...
  return {
    ...
    spouseAnnualIncome: form.spouseAnnualIncome ?? (form.spouseSalary || 0) * 12,
...
function totalAssets(assets = []) {
  return assets.reduce((sum, asset) => sum + clamp(asset.amount, 0, 100000000000), 0);
}
...
function principalFromMonthlyPayment(monthlyPaymentLimit, annualRate, years) {
  ...
  return monthlyPaymentLimit * ((1 + r) ** months - 1) / (r * (1 + r) ** months);
}
...
function evaluatePolicy(ctx, policy) {
  ...
  const maxLoanAtMaxPurchase = Math.max(0, Math.min(productLimit, debtLimit, maxPurchase * ltv, maxPurchase - ctx.availableAssets));
  ...
  const limiting = targetCaps.reduce((min, item) => (item.amount < min.amount ? item : min), targetCaps[0]);
  const eligibilityReasons = policy.eligibility(ctx);
  const reasons = [...eligibilityReasons];

  if (ctx.targetHomePrice > priceLimit) {
    reasons.push(`희망 주택가격이 상품 대상 주택가격 ${readableMoney(priceLimit)}을 초과합니다.`);
  }
  if (targetLoanNeed > productLimit) {
    reasons.push(`필요 대출액이 ${policy.name}의 상품별 대출한도 ${readableMoney(productLimit)}을 초과합니다.`);
  }
  if (targetLoanNeed > ctx.targetHomePrice * ltv) {
    reasons.push(`필요 대출액이 LTV ${Math.round(ltv * 100)}% 한도를 초과합니다.`);
  }
  ...
  return {
    ...
    targetLoanNeed,
    targetPossibleLoan: Math.min(productLimit, ctx.targetHomePrice * ltv, debtLimit, ctx.targetHomePrice),
    limitingLabel: capLabel(limiting.key),
    limitingAmount: limiting.amount === Infinity ? maxLoanAtMaxPurchase : limiting.amount,
    reasons,
    notes: policy.notes
  };
}
...
function policyEngines() {
  const firstHomeLtv = (ctx) => (ctx.firstHome && ctx.region !== 'regulated' ? 0.8 : 0.7);
  const fundCommonEligibility = (ctx, incomeLimit, homePriceLimit) => {
    const reasons = [];
    if (!ctx.isHomeless) reasons.push('무주택 세대주 요건을 충족해야 합니다.');
    if (ctx.annualIncome > incomeLimit) reasons.push(`부부합산 연소득이 ${readableMoney(incomeLimit)} 기준을 초과합니다.`);
    if (ctx.netWorth > 511000000) reasons.push('순자산가액이 2026년 기준 5.11억원을 초과합니다.');
    if (ctx.areaM2 > 85) reasons.push('전용면적 85㎡ 이하 주택 요건을 초과합니다.');
    if (ctx.targetHomePrice > homePriceLimit) reasons.push(`대상 주택가격 ${readableMoney(homePriceLimit)} 기준을 초과합니다.`);
    return reasons;
  };
  ...
  return [
    ...
    },
    {
      ...
      rate: 4.15,
      eligibility: (ctx) => [
        ...fundCommonEligibility(ctx, ctx.isNewlywed ? 85000000 : 70000000, 600000000),
        ...(!ctx.isNewlywed && ctx.childrenCount < 2 ? ['신혼가구 또는 2자녀 이상 조건이 필요합니다.'] : [])
      ],
    ...
    },
    {
      ...
      rate: 4.5,
      eligibility: (ctx) => {
        const incomeLimit = ctx.married === 'married' && ctx.ownAnnualIncome > 0 && ctx.spouseAnnualIncome > 0 ? 200000000 : 130000000;
        return [
          ...fundCommonEligibility(ctx, incomeLimit, 900000000),
          ...(!ctx.hasNewborn ? ['대출접수일 기준 2년 내 출산 또는 입양 조건이 필요합니다.'] : [])
        ];
      },
      notes: '신생아 특례는 2023년 1월 1일 이후 출생아 및 2년 내 출산 조건을 반영했습니다.'
    }
  ];
}
...
function buyerReport(form) {
  ...
  const spouseAnnualIncome = form.married === 'married' ? clamp(form.spouseAnnualIncome, 0, 10000000000) : 0;
  const annualIncome = ownAnnualIncome + spouseAnnualIncome;
  const childrenCount = Math.max(Number(form.childrenCount) || 0, form.multiChild ? 3 : 0);
  const ctx = {
    ...form,
    availableAssets,
    ownAnnualIncome,
    ...
    isNewlywed: form.married === 'married' && (Number(form.marriageYears) || 99) <= 7,
    childrenCount,
    hasNewborn: form.newborn || isRecentBirth(form.birthDate)
  };
  const policyResults = policyEngines().map((policy) => evaluatePolicy(ctx, policy));
  const eligibleResults = policyResults.filter((result) => result.eligible);
  const targetPossibleResults = policyResults.filter((result) => result.possible);
  const best = (eligibleResults.length ? eligibleResults : policyResults)
    .reduce((current, result) => (result.maxPurchase > current.maxPurchase ? result : current), policyResults[0]);
  const homePrice = best.maxPurchase;
  const loan = best.maxLoan;
  const brokerage = Math.min(homePrice * 0.004, homePrice >= 900000000 ? homePrice * 0.005 : homePrice * 0.004);
  const acquisitionBase = homePrice * (form.firstHome === 'yes' ? 0.009 : 0.012);
  ...
  return {
    ...
    benefits: [
      ...
      best.possible ? `${best.name} 기준 최대 가능액이 가장 큼` : '입력 조건상 가능 상품이 없어 불가 사유를 확인해야 함'
    ].filter(Boolean)
  };
}
...
function normalizeLoans() {
  ...
  if (Array.isArray(savedLoans) && savedLoans.length > 0) {
    return savedLoans.map((loan) => ({ ...loan, prepayments: loan.prepayments || [] }));
  }
...
function loanReport(loan) {
  ...
  const maturity = new Date(loan.maturityYear, loan.maturityMonth - 1, loan.payDay);
  const elapsed = Math.max(0, monthsBetween(start, today) + (today.getDate() >= loan.payDay ? 1 : 0));
  ...
  const baseMonthly = loan.method === 'bullet'
    ? monthlyInterest
    : loan.method === 'equalPrincipal'
      ? remainingPrincipal / remainingMonths + monthlyInterest
      : monthlyPaymentByMonths(remainingPrincipal, loan.rate, remainingMonths);
  const progress = loan.principal > 0 ? clamp((paidPrincipal / loan.principal) * 100, 0, 100) : 0;
  ...
  const remainingInterest = Math.max(0, remainingMonths * baseMonthly - remainingPrincipal);
  const savedByPrepay = prepaymentTotal * (loan.rate / 100) * Math.min(8, remainingMonths / 12) * 0.55;
...
function portfolioReport(loans) {
  ...
  const nearest = reports.reduce((best, item) => (!best || item.report.dday < best.report.dday ? item : best), null);
  return {
    reports,
    nearest,
    totalPrincipal: loans.reduce((sum, loan) => sum + loan.principal, 0),
    totalRemaining: reports.reduce((sum, item) => sum + item.report.remainingPrincipal, 0),
    totalMonthly: reports.reduce((sum, item) => sum + item.report.baseMonthly, 0),
    totalPrepaid: reports.reduce((sum, item) => sum + item.report.prepaymentTotal, 0)
  };
}
      ...
      <Shell>
        {!onboarded ? (
          ...
          <>
            ...
            <TabBar>
              <TabButton active={activeTab === 'buy'} onClick={() => switchTab('buy')}>
                <Home size={20} /> 너도 살 수 있어
              </TabButton>
              <TabButton active={activeTab === 'loan'} onClick={() => switchTab('loan')}>
...
function Onboarding({ onSelect }) {
  return (
    <OnboardingWrap>
      ...
      <HeroCopy>사기 전에는 현실적인 한도를, 사고 난 후에는 연체 없는 상환 루틴을 잡아드려요.</HeroCopy>
      <ChoiceGrid>
        <ChoiceCard onClick={() => onSelect('buy')}>
          <IconOrb><Home size={34} /></IconOrb>
          <h2>나에게 맞는 집 찾기</h2>
          <p>내 진짜 자산으로 가능한 매수 금액과 부대비용을 먼저 확인해요.</p>
          <ChevronRight />
        </ChoiceCard>
        <ChoiceCard onClick={() => onSelect('loan')}>
          <IconOrb><WalletCards size={34} /></IconOrb>
          <h2>받은 대출 관리하기</h2>
...
function BuyerTab({ onAd }) {
  const [form, setForm] = useState(() => normalizeBuyerForm(load(STORAGE_KEYS.buyer, initialBuyer)));
  const [report, setReport] = useState(() => buyerReport(normalizeBuyerForm(load(STORAGE_KEYS.buyer, initialBuyer))));
  ...
  const updateAsset = (id, patch) => {
    update({
      assets: form.assets.map((asset) => (asset.id === id ? { ...asset, ...patch } : asset))
    });
  };
  ...
  return (
    <Stack>
      <Duo>
        <Panel>
          <PanelTitle><Calculator size={20} /> 매수 가능 금액 계산기</PanelTitle>
          <Field label="연소득(세전)">
            <MoneyInput value={form.annualIncome} onChange={(value) => update({ annualIncome: value })} />
          </Field>
          <Segment label="결혼 유무" value={form.married} onChange={(married) => update({ married })} options={[
            ['single', '미혼'],
            ['married', '기혼']
          ]} />
          {form.married === 'married' && (
            <>
              <Field label="배우자 연소득(세전)">
                <MoneyInput value={form.spouseAnnualIncome || 0} onChange={(value) => update({ spouseAnnualIncome: value })} />
              </Field>
              <Field label="혼인 기간">
                <NumberInput value={form.marriageYears} min="0" onChange={(value) => update({ marriageYears: value })} suffix="년" />
              </Field>
            </>
          )}
          <ControlWrap>
            ...
            <AssetList>
              {form.assets.map((asset) => (
                <AssetRow key={asset.id}>
                  ...
                  />
                  <MoneyInput value={asset.amount} onChange={(amount) => updateAsset(asset.id, { amount })} />
                  <IconButton title="자산 삭제" onClick={() => removeAsset(asset.id)} disabled={form.assets.length <= 1}>
                    <X size={16} />
                  </IconButton>
                </AssetRow>
              ))}
            </AssetList>
            <AssetTotal>총 {readableMoney(totalAssets(form.assets))}</AssetTotal>
          </ControlWrap>
          <Field label="순자산">
            <MoneyInput value={form.netWorth} onChange={(value) => update({ netWorth: value })} />
          </Field>
          <Field label="기존 부채 월상환액">
            <MoneyInput value={form.existingDebtMonthly} onChange={(value) => update({ existingDebtMonthly: value })} />
          </Field>
          <Field label="희망 주택가격">
            <MoneyInput value={form.targetHomePrice} onChange={(value) => update({ targetHomePrice: value })} />
          </Field>
          <TwoCols>
            <Field label="전용면적">
              <NumberInput value={form.areaM2} min="0" onChange={(value) => update({ areaM2: value })} suffix="㎡" />
            </Field>
            <Field label="자녀 수">
              <NumberInput value={form.childrenCount} min="0" onChange={(value) => update({ childrenCount: value, multiChild: value >= 3 })} suffix="명" />
            </Field>
          </TwoCols>
          <Segment label="무주택 여부" value={form.isHomeless} onChange={(isHomeless) => update({ isHomeless })} options={[
            ['yes', '무주택'],
            ['no', '유주택']
          ]} />
          <Segment label="생애최초 주택구입" value={form.firstHome} onChange={(firstHome) => update({ firstHome })} options={[
            ['yes', 'Y'],
            ['no', 'N']
          ]} />
          <Field label="최근 출산일">
            <TextInput type="date" value={form.birthDate || ''} onChange={(e) => update({ birthDate: e.target.value, newborn: isRecentBirth(e.target.value) })} />
          </Field>
          <ToggleRow icon={<Sparkles size={18} />} label="최근 2년 이내 출산" checked={form.newborn} onChange={(newborn) => update({ newborn })} />
          <Segment label="지역" value={form.region} onChange={(region) => update({ region })} options={[
            ['normal', '비규제지역'],
            ['regulated', '규제지역']
          ]} />
          <PrimaryButton onClick={calculate}><BadgePercent size={18} /> 얼만지 계산하기</PrimaryButton>
        </Panel>

        <Panel emphasis>
          <PanelTitle><PiggyBank size={20} /> 결과 가이드 리포트</PanelTitle>
          <BigResult>최대 {eok(report.homePrice)} 원대</BigResult>
          <ResultCopy>
            {report.hasTargetPossiblePolicy
              ? `희망 주택가격은 ${report.policyResults.filter((policy) => policy.possible).map((policy) => policy.name).join(', ')} 기준으로 검토 가능합니다.`
              : `희망 주택가격은 현재 조건으로 어렵고, ${report.bestPolicy.name} 기준 최대 매수 가능 범위를 표시합니다.`}
          </ResultCopy>
          <MetricGrid>
            <Metric label="부부합산 연소득(세전)" value={readableMoney(report.annualIncome)} />
            <Metric label="유용 가능 자산" value={readableMoney(report.availableAssets)} />
            <Metric label="예상 대출" value={eok(report.loan)} />
            <Metric label="적용 LTV" value={`${Math.round(report.ltv * 100)}%`} />
            ...
            {report.policyResults.map((policy) => (
              <PolicyCard key={policy.id} possible={policy.possible}>
                <PolicyTop>
                  ...
                  <PolicyBadge possible={policy.possible}>{policy.possible ? '가능' : '불가'}</PolicyBadge>
                </PolicyTop>
                <MetricGrid>
                  <Metric label="최대 매수가" value={readableMoney(policy.maxPurchase)} />
                  <Metric label="최대 대출" value={readableMoney(policy.maxLoan)} />
                  <Metric label="병목 한도" value={policy.limitingLabel} />
                  <Metric label="한도 금액" value={readableMoney(policy.limitingAmount)} />
                </MetricGrid>
                {policy.reasons.length > 0 ? (
                  <ReasonList>
                    {policy.reasons.map((reason) => <li key={reason}>{reason}</li>)}
                  </ReasonList>
                ) : (
          ...
          <PayRow title="원리금 균등" desc="매월 균등하게 내는 돈" amount={won(report.equalPayment)} />
          <PayRow title="원금 균등" desc="첫 달 가장 많이 내는 돈" amount={won(report.equalPrincipalFirst)} />
          <PayRow title="만기 일시" desc="매월 이자만 내는 돈" amount={won(report.interestOnly)} />
          {showBenefits && (
            <BenefitBox>
              <Gem size={18} />
              <div>
                <strong>맞춤형 정책 안내</strong>
                <p>{report.benefits.length ? report.benefits.join(' · ') : '현재 입력 조건으로는 기본 대출 기준을 우선 확인하세요.'}</p>
              </div>
            </BenefitBox>
          )}
          <DisclaimerBox>
            ...
            <p>
              본 결과는 입력값과 공개 정책 요건을 바탕으로 한 참고용 시뮬레이션입니다. 실제 대출 가능 여부, 한도, 금리, 세금, 중개보수는 은행 심사, 보증기관 심사, 차주 신용도, 기존 부채, 스트레스 DSR, 지역·시점별 규제, 주택의 실제 평가액, 정책 변경 및 예외 요건에 따라 달라질 수 있습니다.
            </p>
            <p>
              일반 주담대는 은행권 간편 기준으로 계산되며 금융회사별 내부 심사를 대체하지 않습니다. 디딤돌·신생아 특례 등 정책상품도 최종 판단은 주택도시기금, 수탁은행, 관계 기관의 최신 고시와 심사 결과를 우선합니다. 본 앱의 계산 결과만을 근거로 계약, 대출 신청, 투자, 세무 의사결정을 진행하지 마세요.
            </p>
          </DisclaimerBox>
        </Panel>
      </Duo>
...
function LoanTab() {
  ...
  const addPrepayment = (loanId) => {
    ...
    const next = loans.map((loan) => (
      loan.id === loanId ? { ...loan, prepayments: [item, ...(loan.prepayments || [])] } : loan
    ));
    persistLoans(next);
  ...
  return (
    <Stack>
      ...
      <Duo>
        <ActionPanel>
          <PanelTitle><Landmark size={20} /> 내 대출 포트폴리오</PanelTitle>
        ...
        <Panel emphasis>
          <PanelTitle><CircleDollarSign size={20} /> 전체 상환 대시보드</PanelTitle>
          <Gauge>
            <GaugeTop>
              <span>전체 대출 중 남은 원금</span>
              <strong>{eok(portfolio.totalRemaining)}</strong>
            </GaugeTop>
            <GaugeTrack><GaugeFill style={{ width: `${portfolio.totalPrincipal ? 100 - (portfolio.totalRemaining / portfolio.totalPrincipal) * 100 : 0}%` }} /></GaugeTrack>
          </Gauge>
          <MetricGrid>
            <Metric label="등록 대출" value={`${loans.length}개`} />
            <Metric label="이번 달 총 출금" value={won(portfolio.totalMonthly)} />
            <Metric label="누적 중도상환" value={won(portfolio.totalPrepaid)} />
            <Metric label="해방 포인트" value={`${points}P`} />
          </MetricGrid>
          <PrimaryButton onClick={certify}><CheckCircle2 size={18} /> 이번 달 정상 상환 인증</PrimaryButton>
        </Panel>
      </Duo>
          ...
          <LoanCard key={loan.id}>
            <LoanCardTop>
              <div>
                ...
                <span>{loan.type === 'mortgage' ? '주택담보대출' : '신용대출'} · {loan.rate}%</span>
              </div>
              <CardActions>
                <IconButton title="대출 수정" onClick={() => setEditingLoan({ ...loan, prepayments: loan.prepayments || [] })}>
                  <Pencil size={17} />
                </IconButton>
                <DueBadge urgent={report.dday <= 3}>{report.dday === 0 ? '오늘' : `D-${report.dday}`}</DueBadge>
              </CardActions>
            </LoanCardTop>
            <PayRow title={`${loan.payDay}일 출금`} desc={`${report.nextPay.toLocaleDateString('ko-KR')} 예정`} amount={won(report.baseMonthly)} />
            <MetricGrid>
              <Metric label="남은 원금" value={won(report.remainingPrincipal)} />
              <Metric label="만기일" value={`${loan.maturityYear}.${String(loan.maturityMonth).padStart(2, '0')}`} />
              <Metric label="상환 회차" value={`${report.elapsed} / ${report.totalMonths}`} />
              <Metric label="진행률" value={`${Math.round(report.progress)}%`} />
            </MetricGrid>
            <GaugeTrack><GaugeFill style={{ width: `${report.progress}%` }} /></GaugeTrack>
            <SectionSub>이 대출 중도상환</SectionSub>
            <InlineAction>
              <MoneyInput value={prepayValues[loan.id] || 1000000} onChange={(amount) => setPrepayValues({ ...prepayValues, [loan.id]: amount })} />
              <GoldButton onClick={() => addPrepayment(loan.id)}><HandCoins size={18} /> 입력</GoldButton>
            </InlineAction>
            <BenefitBox>
              <Trophy size={18} />
              <div>
                <strong>원금 감소</strong>
                <p>누적 중도상환 {won(report.prepaymentTotal)} · 예상 이자 절감 {won(report.savedByPrepay)}</p>
              </div>
            </BenefitBox>
          </LoanCard>
        ))}
    ...
    <Overlay onClick={onClose}>
      <EditorModal onClick={(e) => e.stopPropagation()}>
        <ModalHeader>
          ...
          <IconButton title="닫기" onClick={onClose}><X size={18} /></IconButton>
        </ModalHeader>

        <Field label="대출 이름">
          <TextInput value={loan.name} onChange={(e) => update({ name: e.target.value })} />
        </Field>
        <Field label="총 대출 원금">
          <MoneyInput value={loan.principal} onChange={(principal) => update({ principal })} />
        </Field>
        <TwoCols>
          <Field label="대출 금리">
            <TextInput type="number" value={loan.rate} step="0.1" onChange={(e) => update({ rate: Number(e.target.value) })} />
          </Field>
          <Field label="매달 출금일">
            <TextInput type="number" min="1" max="28" value={loan.payDay} onChange={(e) => update({ payDay: clamp(e.target.value, 1, 28) })} />
          </Field>
        </TwoCols>
        <Segment label="대출 유형" value={loan.type} onChange={(type) => update({ type })} options={[
          ['mortgage', '주담대'],
          ['credit', '신용대출']
        ]} />
        <Segment label="상환 방식" value={loan.method} onChange={(method) => update({ method })} options={[
          ['equalPayment', '원리금'],
          ['equalPrincipal', '원금균등'],
          ['bullet', '만기일시']
        ]} />
        ...
        <PickerGrid>
          <YearSelect value={loan.startYear} onChange={(startYear) => update({ startYear })} />
          <MonthSelect value={loan.startMonth} onChange={(startMonth) => update({ startMonth })} />
        </PickerGrid>
        <PickerLabel>대출 만기 연/월</PickerLabel>
        <PickerGrid>
          <YearSelect value={loan.maturityYear} future onChange={(maturityYear) => update({ maturityYear })} />
          <MonthSelect value={loan.maturityMonth} onChange={(maturityMonth) => update({ maturityMonth })} />
        </PickerGrid>

        <ModalFooter>
          <GhostButton onClick={onClose}>취소</GhostButton>
          <PrimaryButton onClick={() => onSave({ ...loan, name: loan.name.trim() || '이름 없는 대출' })}>
            저장하기
          </PrimaryButton>
        </ModalFooter>
      </EditorModal>
...
function YearSelect({ value, onChange, future = false }) {
  ...
  const years = Array.from({ length: future ? 35 : 12 }, (_, idx) => future ? base + idx : base - idx);
  return (
...
function MonthSelect({ value, onChange }) {
  return (
    <Select value={value} onChange={(e) => onChange(Number(e.target.value))}>
      {Array.from({ length: 12 }, (_, idx) => idx + 1).map((month) => <option key={month} value={month}>{month}월</option>)}
    </Select>
  );
}
    ...
    <ControlWrap>
      ...
      <SegmentWrap>
        {options.map(([key, text]) => (
          <SegmentItem key={key} active={value === key} onClick={() => onChange(key)}>{text}</SegmentItem>
        ))}
      </SegmentWrap>
    </ControlWrap>
  );
...
function ToggleRow({ icon, label, checked, onChange }) {
  return (
    <ToggleLine onClick={() => onChange(!checked)}>
      ...
      <Switch checked={checked}><i /></Switch>
    </ToggleLine>
  );
}
  ...
  return (
    <InputWithAssist>
      <TextInput
        ...
        onChange={(e) => onChange(clamp(e.target.value.replaceAll(',', ''), 0, max))}
      />
...
function NumberInput({ value, onChange, suffix, min = '0' }) {
  return (
    <InlineNumber>
      <TextInput type="number" min={min} value={value} onChange={(e) => onChange(Number(e.target.value))} />
      <span>{suffix}</span>
    </InlineNumber>
  );
}
...
const Panel = styled.section`
  ...
  border-radius: 8px;
  padding: 18px;
  box-shadow: ${({ emphasis }) => emphasis ? '0 16px 36px rgba(16, 26, 45, 0.10)' : 'none'};
`;
...
const BenefitBox = styled.div`
  ...
  svg { color: ${gold}; flex: 0 0 auto; }
  p { margin: 4px 0 0; color: ${slate}; font-size: 13px; line-height: 1.45; }
`;
...
const PolicyCard = styled.div`
  border: 1px solid ${({ possible }) => possible ? 'rgba(17, 166, 131, 0.36)' : 'rgba(217, 74, 66, 0.34)'};
  border-radius: 8px;
  padding: 13px;
...
const LoanCard = styled.article`
  ...
  border-radius: 8px;
  padding: 16px;
  box-shadow: ${({ selected }) => selected ? '0 12px 28px rgba(216, 170, 60, 0.16)' : 'none'};
`;
...
const Dday = styled.div`
  ...
  animation: ${({ urgent }) => urgent ? 'pulse 1s infinite alternate' : 'none'};

  @keyframes pulse {
    from { filter: saturate(1); }
    to { filter: saturate(1.45); }
  }
